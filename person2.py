...
class Person:  # класс персонажа
    ...
    def defend(self):  # функция обороны
        ...
        return '{} встал в защитную стойку.\nЕго защита увеличена в 1.5 раза, уклонение в 1.2 раза\n'.format(self.name)
    ...
    def set_damage(self, att):  # функция получения урона
        ...
        if chance_to_evade > self.evasion:  # если шанс попадания не больше уклонения
            ...
            damage = att - (self.out_protec * critism_of_damage)  # формула, высчитывающая нанесенный урон
            if damage < 0:
                damage = 0
            ...
            the_ans += '''Выпало случайное число {} > {} (увертливость соперника)
Удар нанес урона {} (урон - (броня * коэффициент критичности ({})))\n'''.format(chance_to_evade, self.evasion, damage,
                                                                                critism_of_damage)
        # если шанс попадания входит в диапазон [0, evasion] (Промах)
        else:
            the_ans += '''Выпало случайное число {} <= {} (увертливость соперника)
Промах\n'''.format(chance_to_evade, self.evasion)
        return the_ans
